...
$(document).on('click', '.sidebar-menu-bar', function () {
 ...
 // Check on page load if the window is in mobile
if ($(window).width() < 768) { // Adjust the breakpoint as necessary
  if (!$('.sidebar').hasClass('active')) {
    console.log("togle")
    toggleSidebar();
  }
}
// Trigger sidebar toggle on window resize
$(window).on('resize', function () {
  if ($(window).width() < 768) { // Adjust the breakpoint as necessary
      ...
      // Optionally, ensure the sidebar is open on larger screens
      if (!$('.sidebar').hasClass('active')) {
          toggleSidebar();
      }
  }
