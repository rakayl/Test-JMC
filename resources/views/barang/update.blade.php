...
@section('content')
  <form class="card-form" method="POST" enctype="multipart/form-data" action="{{ setRoute('barang.update',$data->id) }}">
     @csrf
    @method("POST")
                ...
                <div class="row">
                    @if(auth()->user()->hasRole('admin'))
                         <div class="row">
                                <div class="col-xl-6 col-lg-6 form-group">
                                    ...
                                    <option @if($data->user_id == $va->id) selected @endif value="{{ $va->id }}">{{ $va->name }}</option>
                                    @endforeach
                                </select>
                                </div>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-xl-12 col-lg-12 form-group">
                            ...
                            <input type="text" class="form--control" value="{{$data->asal_barang}}" name="asal_barang" id="asal_barang" required>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xl-6 col-lg-6 form-group">
                            <label>{{ __('Nomor Surat') }}*</label>
                            <input type="text" class="form--control" id="no_surat" value="{{$data->no_surat}}" name="no_surat">
                        </div>
                        <div class="col-xl-6 col-lg-6 form-group">
...
@push('script')
   <script>
        const dataToUpdate = <?php echo json_encode($data); ?>;;
        document.addEventListener('DOMContentLoaded', function() {
            ...
            kategoriData.forEach(function(kategori) {
             ...
             kategoriSelect.value = dataToUpdate.kategori_id;
            
            // Trigger change event untuk memuat subkategori
            const event = new Event('change');
            kategoriSelect.dispatchEvent(event);
            kategorisUpdate(dataToUpdate.kategori_id);
             setTimeout(() => {
                dataToUpdate.detail.forEach((item, index) => {
                    tambahBaris(index === 0, item);
                });
            }, 500);
            ...
            //
             function kategorisUpdate(value) {
                 const selectedKategoriId = value;
                subkategoriSelect.innerHTML = '<option value="">Pilih Subkategori</option>';
                subkategoriSelect.disabled = true;
                batasanHargaInput.value = '';
                currentMaxPrice = 0;
                        ...
                        .then(data => {
                            ...
                            if (data && data.length > 0) {
                                ...
                                data.forEach(function(item) {
                                    ...
                                    subkategoriSelect.value = dataToUpdate.sub_kategori_id;     
            ...
            function tambahBaris(isFirstRow = false, itemData = null) {
                const newRow = document.createElement('tr');
                if (itemData) {
                    newRow.dataset.itemId = itemData.id;
                }
                newRow.innerHTML = `
                    <td><input type="hidden" name="id_detail[]" value="${itemData ? itemData.id : ''}" required><input type="text" name="nama_barang[]" value="${itemData ? itemData.nama_barang : ''}" required></td>
                    <td class="input-rupiah">
                     <input type="text" name="harga[]" class="harga" required 
                               oninput="formatRupiahInput(this)" value="${itemData ? formatNumber(itemData.harga) : ''}">
                    </td>
                    <td><input type="number" name="jumlah[]"  class="jumlah" min="1" value="${itemData ? itemData.jumlah_barang : 1}" required></td>
                    <td><input type="text" name="satuan[]" value="${itemData ? itemData.satuan : ''}" required></td>
                    <td><input type="text" name="total[]" class="total" value="Rp 0" readonly></td>
                    <td><input type="date" name="expired[]" value="${itemData && itemData.expired_date ? itemData.expired_date.split('T')[0] : ''}"></td>
                ...
                counter++;
                 if (itemData) {
                    ...
                    hargaInput.value = itemData.harga;
                    jumlahInput.value = itemData.jumlah_barang;
                    
                    // Trigger input untuk menghitung total
                    if (hargaInput.value) {
...
//                        updateNomorBaris();
            ...
            function hitungTotal() {
                ...
                const harga = getNumericValue(row.querySelector('.harga').value) || 0;
                const jumlah = getNumericValue(row.querySelector('.jumlah').value) || 0;
